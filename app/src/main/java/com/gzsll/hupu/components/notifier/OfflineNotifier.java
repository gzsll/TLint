 ...
 */
@PerService public class OfflineNotifier extends Notifier {

  private Context mContext;

  @Inject public OfflineNotifier(Context mContext) {
    this.mContext = mContext;
  }

  public void notifyThreads(Forum forum, long offlineLength) {
    String title = String.format("正在离线板块[%s]", forum.getName());
    String content = String.format("节省流量%s", FormatUtils.formatFileSize(offlineLength));
  ...
  public void notifyThreadsSuccess(int forumSize, int threadsSize, long threadsLength) {
    ...
    String content =
        String.format("共%d篇帖子，节省流量%s", threadsSize, FormatUtils.formatFileSize(threadsLength));
  ...
  public void notifyPictureSuccess(int picSize, long picLength) {
    ...
    String content = String.format("%s张图片，节省流量%s", String.valueOf(picSize),
        FormatUtils.formatFileSize(picLength));
  ...
  public void notifyReplies(Thread thread, long replyLength) {
    ...
    String content = String.format("节省流量%s", FormatUtils.formatFileSize(replyLength));
  ...
  public void notifyRepliesSuccess(int threadSize, int replySize, long replyLength) {
    ...
    String content =
        String.format("共%d篇回复，节省流量%s", replySize, FormatUtils.formatFileSize(replyLength));
