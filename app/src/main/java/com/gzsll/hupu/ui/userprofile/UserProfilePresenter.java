 ...
 */
@PerActivity public class UserProfilePresenter implements UserProfileContract.Presenter {
  ...
  @Inject public UserProfilePresenter(GameApi gameApi, String uid) {
    mGameApi = gameApi;
    this.uid = uid;
  }

  @Override public void receiveUserInfo() {
    mUserProfileView.showLoading();
    mSubscription = mGameApi.getUserInfo(uid).map(new Func1<UserData, UserResult>() {
      @Override public UserResult call(UserData userData) {
        if (userData != null) {
      ...
      @Override public void call(UserResult userResult) {
  ...
  @Override public void attachView(@NonNull UserProfileContract.View view) {
