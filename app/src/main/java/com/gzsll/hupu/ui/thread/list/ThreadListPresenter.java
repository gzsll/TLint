...
@PerActivity public class ThreadListPresenter implements ThreadListContract.Presenter {
  ...
  private void loadSearchList() {
    ...
    mGameApi.search(key, fid, pageIndex).map(new Func1<SearchData, List<Thread>>() {
      @Override public List<Thread> call(SearchData searchData) {
        if (searchData != null) {
          if (pageIndex == 1) {
            threads.clear();
          }
  ...
  private void getAttendStatus() {
    mForumApi.getAttentionStatus(fid)
        ...
        .subscribe(new Action1<AttendStatusData>() {
          @Override public void call(AttendStatusData attendStatusData) {
  ...
  @Override public void onAttentionClick() {
    if (isLogin()) {
      if (isAttention) {
        delAttention();
      } else {
