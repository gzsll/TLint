  ...
  private void loadMessageList(final boolean clear) {
    mSubscription = mForumApi.getMessageList(lastTid, page)
        ...
        .doOnNext(new Action1<MessageData>() {
          @Override public void call(MessageData messageData) {
            if (clear) {
              messages.clear();
            }
          }
        ...
        .map(new Func1<MessageData, List<Message>>() {
          @Override public List<Message> call(MessageData messageData) {
        ...
        .subscribe(new Action1<List<Message>>() {
          @Override public void call(List<Message> messages) {
            if (messages != null) {
              if (messages.isEmpty()) {
