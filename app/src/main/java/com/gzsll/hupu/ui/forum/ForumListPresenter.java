 ...
 */
@PerActivity public class ForumListPresenter implements ForumListContract.Presenter {
  ...
  @Override public void onForumListReceive(final String forumId) {
  ...
  private Observable<List<Forum>> getForumListObservable(final String forumId) {
    Observable<List<Forum>> firstObservable = Observable.fromCallable(new Func0<List<Forum>>() {
      @Override public List<Forum> call() {
  ...
  private void loadUserForums() {
    mForumApi.getMyForums().doOnNext(new Action1<MyForumsData>() {
      @Override public void call(MyForumsData result) {
      ...
      @Override public void call(MyForumsData myForumsData) {
  ...
  private void loadAllForums(final String forumId) {
    mForumApi.getForums().doOnNext(new Action1<ForumsData>() {
      @Override public void call(ForumsData result) {
        if (result != null) {
      ...
      @Override public void call(ForumsData forumsData) {
  ...
  @Override public void attachView(@NonNull ForumListContract.View view) {
    mForumListView = view;
  }
