 ...
 */
@PerActivity public class PmListPresenter implements PmListContract.Presenter {

  private GameApi mGameApi;

  @Inject public PmListPresenter(GameApi gameApi) {
    mGameApi = gameApi;
  }
  ...
  @Override public void onPmListReceive() {
  ...
  private void loadPmList(final boolean clear) {
    mSubscription =
        mGameApi.queryPmList(lastTime).subscribeOn(Schedulers.io()).doOnNext(new Action1<PmData>() {
          @Override public void call(PmData pmData) {
            if (clear) {
              mPms.clear();
            }
          }
