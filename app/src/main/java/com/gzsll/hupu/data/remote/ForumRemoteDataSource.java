...
public class ForumRemoteDataSource implements ForumDataSource {
  ...
  @Override public Observable<List<Forum>> getForumList(final String forumId) {
    if (forumId.equals("0")) {
      return mForumApi.getMyForums().map(new Func1<MyForumsData, List<Forum>>() {
        @Override public List<Forum> call(MyForumsData myForumsData) {
        ...
        @Override public List<Forum> call(Throwable throwable) {
          return null;
        }
      });
    } else {
      return mForumApi.getForums().map(new Func1<ForumsData, List<Forum>>() {
        @Override public List<Forum> call(ForumsData forumsData) {
          if (forumsData != null) {
