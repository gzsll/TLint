...
public class ContentRemoteDataSource implements ContentDataSource {
  ...
  @Override public Observable<ThreadInfo> getThreadInfo(String fid, String tid) {
    return mForumApi.getThreadInfo(tid, fid, 1, "")
        .onErrorReturn(new Func1<Throwable, ThreadInfo>() {
          @Override public ThreadInfo call(Throwable throwable) {
            return null;
          }
        })
  ...
  @Override public Observable<List<ThreadReply>> getReplies(String fid, String tid, int page) {
    return mForumApi.getThreadReplyList(tid, fid, page)
        .map(new Func1<ThreadReplyData, List<ThreadReply>>() {
          @Override public List<ThreadReply> call(ThreadReplyData threadReplyData) {
        ...
        .onErrorReturn(new Func1<Throwable, List<ThreadReply>>() {
          @Override public List<ThreadReply> call(Throwable throwable) {
            return null;
          }
        })
        .subscribeOn(Schedulers.io());
  ...
  @Override public Observable<List<ThreadReply>> getLightReplies(String fid, String tid) {
    return mForumApi.getThreadLightReplyList(tid, fid)
        .map(new Func1<ThreadLightReplyData, List<ThreadReply>>() {
          @Override public List<ThreadReply> call(ThreadLightReplyData threadLightReplyData) {
